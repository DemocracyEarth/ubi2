...
The app ships the verified Base Sepolia, Ethereum Sepolia, Celo Sepolia, and Robinhood Chain
Testnet contract pairs as public defaults. They can be overridden with `NEXT_PUBLIC_*` variables,
but the server still fails closed unless its issuer signer matches both live `issuer()` getters.
World Chain Sepolia and every mainnet remain zero-addressed and visibly unavailable. See the public
