...
//! against TWO independent primary sources (EC-C7 research):
//!   * GitHub `contracts/contracts/verifiers/disclose/Verifier_vc_and_disclose.sol` (generated verifier),
//!   * the Celoscan-VERIFIED live Celo-mainnet deployment `0x0A57C317800865194496763377d25CA2082DB649`
//!     (byte-identical to the Celo-Sepolia staging verifier `0x7C2FBA7F...`, so one VK serves both).
//! The G2 coordinates are emitted in snarkjs `[[c0,c1],...]` order (the `.sol` stores `c1,c0`); the swap
//! direction was proven by reducing `vk_gamma_2` to the canonical BN254 G2 generator, and all 4 fixed
//! points + 22 IC points pass on-curve + prime-order-subgroup validation.
//!
...
fn parse_prod_vk() -> (SnarkjsVk, usize) {
    ...
    assert_eq!(j["nPublic"], 21, "production vc_and_disclose is a 21-signal circuit");
    let ic = j["IC"]
        .as_array()
        .unwrap()
        .iter()
...
fn prod_vk_imports_at_confirmed_arity() {
    ...
    assert_eq!(ic_len, 22, "vc_and_disclose VK must carry 22 IC points (21 signals + 1)");
    ...
    let reloaded = PinnedVerifyingKey::from_canonical_bytes(&bytes)
        .expect("canonical bytes must re-load with full validation (on-curve, subgroup, canonical)");
